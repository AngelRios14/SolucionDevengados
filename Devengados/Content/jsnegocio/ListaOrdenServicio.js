...
$(document).ready(function () {
    ...
    fnListaTpComprobantes();
    

    $("#ddlTpComprobante").change(function () {
        fnCambiarNombre();
    });
});
...
function fnInicializarValidaciones() {
    ...
    $("#frmEntregable").validate({
        ...
        rules: {
            ...
            txtFacturaES: { "required": false, "Comprobante": true }
            /*ddlTpComprobante: {
                "required": function (element) {
                    return $('#ddlTpComprobante').val() !== "0";
                }      }*/
...
function fnCargarDataTableServicio(LstServicios) {
    ...
    dataTable = $('#dtServicios').DataTable({
        ...
        "columns": [
            { "data": null, "className": "dt-center", "defaultContent": "<button class='btn_tblEditar'  style='border:0;'   title='Detalle'>  <img style='border-width:1px;border-style:None;' src='../Content/Images/btnEdit.gif' /> </button>" },
...
function fnCargarDataTableEntregables(LstEntregables) {        
    ...
    dataTable = $('#dtgEntregablesPlg').DataTable({        
        ...
        "columns": [
            {
                "data": null, "className": "dt-center", "defaultContent": "<button   type='button' class='btn_tblEditarEntregable'  style='border:0'   title='Detalle'>  <img style='border-width:1px;border-style:None;' src='../Content/Images/btnEdit.gif' /> </button>"
                    
            },
            { "data": null, "className": "dt-center", "defaultContent": "<button  type='button' class='btn_tblEliminar' style='border:0'  title='Eliminar'> <img style='border-width:1px;border-style:None;' src='../Content/Images/btnDelete.gif' /></button>" },
...
function fnListaTpComprobantes() {

    var requrl = xpathListaTpComprobantes;
    ...
    $.ajax({
      ...
      //  data: params,
        ...
        success: function (response) {
            ...
            var objLstComprobante = response.DataTpComprobante;
            if (objLstComprobante !== null) {
                if (objLstComprobante.length > 0) {
                    $("#ddlTpComprobante").empty();
                    var nroRegistros = objLstComprobante.length;
                    $("<option value='0'>- Seleccione -</option>").appendTo("#ddlTpComprobante");
                    for (var i = 0; i < nroRegistros; i++) {
                        $("<option value='" + objLstComprobante[i].IdComprobante + "'>" + objLstComprobante[i].Descripcion + "</option>").appendTo("#ddlTpComprobante");
                    }
                }
            }
        },
...
function fnCambiarNombre() {

    
    var tpComp = $("#ddlTpComprobante option:selected").val();
    var txttpComp = $("#ddlTpComprobante option:selected").html();
    if (tpComp === 0 || tpComp === "0")
        $("#lblTpComprobante").text("");
    else
        $("#lblTpComprobante").text(txttpComp);
}
...
$.validator.addMethod(
    ...
    function (value, element) {
        var xvalor = value * 1;
        var xtpc = $.trim($('#ddlTpComprobante').val());
        if (xtpc === 0 || xtpc === "0")
            return true;                
        else
            return false;
